...
Don't ask me any details about Raspberry Pi, I bought my first this week (I am writing this). I had some fun with Arduino and Particle Photon a long time ago, so I know a little about these things.


#### Install Debian 64-bit
For reference I have used https://raspi.debian.net/tested-images/


### Get I2C working
Insert "sudo" where appropriate.
...
#### Test the fan
Now, that we know the real bus, we can start controlling the fan.
```
...
Ultimately it will install its stuff, which I believe you know what to do.

#### Fix the daemon script
Actual work is being done by Python script called argononed.py.
I recommend you copy the script elsewhere (your home), to conduct experiements and to replace the original one later.
```
# cd
...
```
This GPIO library has issues with 64-bit system, apparently. Or maybe just with Debian 64-bit.
If anyone has a better fix, then please let us know.

#### Workaround
After some (surprisingly brief) googling I found Patrick and this thread - https://bugzilla.redhat.com/show_bug.cgi?id=1780903
His fix indeed worked.
```
# cd /boot/firmware/
# nano cmdline.txt
```
Add the "iomem=relaxed" argument, so that my line looked
```
...
```
I could not figure out, how to fix this. Again, anyone please let us know.
